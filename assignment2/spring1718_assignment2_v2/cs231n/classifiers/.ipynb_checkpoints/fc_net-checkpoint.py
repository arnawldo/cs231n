...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                #     self.params[f'beta{i+1}'] = np.ones(hidden_dims[0])
               ...
               # softmax layer
                ...
                # TODO do you need normalization on final layer?
                #if self.normalization == 'batchnorm':
                #    self.params[f'gamma{i+1}'] = np.ones(hidden_dims[-1])
                #    self.params[f'beta{i+1}'] = np.ones(hidden_dims[-1])
            else:
                # other hidden layer
    ...
    def loss(self, X, y=None):
        ...
        # activations A0->input, A1->output from 1st layer, A2->output from 2nd layer ..etc
        A = {'A0': X}
        
        for i in range(self.num_layers-1):
            scores = A[f'A{i}'] @ self.params[f'W{i+1}'] + self.params[f'b{i+1}']
            # # batch normalization
            # normalized_scores = scores * self.params[f'gamma{i+1}'] + self.params[f'beta{i+1}']
            # ReLU
            activations = np.maximum(0, scores)
            
            A[f'A{i+1}'] = activations
        
        # forward pass through softmax layer
        A[f'A{self.num_layers}'] = A[f'A{self.num_layers-1}'] @ self.params[f'W{self.num_layers}'] + self.params[f'b{self.num_layers}']
        scores = A[f'A{self.num_layers}']
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        correct_class_probabilities = exp_shifted_scores[:, y] / np.sum(exp_shifted_scores, axis=1, keepdims=True)
        loss = -1 * np.log(np.sum(correct_class_probabilities))
        ...
        dloss_dscores = correct_class_probabilities
        dloss_dscores[:, y] = dloss_dscores[:, y] - 1
        
        # dloss_dWscores = dscores_dWscores.T . dloss_dscores
        grads[f'W{self.num_layers}'] = A[f'A{self.num_layers-1}'].T @ dloss_dscores
        # dloss_dbscores = sum(dloss_dbscores)
        grads[f'b{self.num_layers}'] = np.sum(dloss_dscores, axis=0)
